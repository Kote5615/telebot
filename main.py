...
def recording_in_vail_with_mats(message):
        # Проверяем, есть ли в ответе сообщение, на которое нужно записать репорт
        if message.reply_to_message is not None:
            message_text = message.reply_to_message.text
            mate.mat.append(message_text)
            bot.send_message(chat_id=message.chat.id, text="Сообщение успешно записано.")
            print(mate.mat)
        else:
            bot.send_message(chat_id=message.chat.id, text="Ответьте на сообщение, чтобы записать репорт.")
